...
if [ -z $sup ]; then
  ...
  for i in {0..9}; do
    num=`qstat -f -u '*' | grep $sup.q@compute-0-$i -A 1 | tail -1 | awk '{print $1}'`
    if [[ $num =~ ^[0-9]+$ ]] ; then
       person=`qstat -f -u '*' | grep $sup.q@compute-0-$i -A 1 | tail -1 | awk '{print $4}'`
       echo -e $i ": ${RED}occupied${RESTORE}, $person"
    else
